...
struct cuvsIvfPqIndexParams {
   ...
   * performance, but lower recall. If 'pq_bits' is 8, 'pq_dim' can be set to
   * any number, but multiple of 8 are desirable for good performance. If
   * 'pq_bits' is not 8, 'pq_dim' should be a multiple of 8. For good
   ...
   * `dim` to `rot_dim`, which may be slightly larger than the original space
   * and and is a multiple of `pq_dim` (`rot_dim % pq_dim == 0`). However, this
   * transform is not necessary when `dim` is multiple of `pq_dim`
   ...
   * training. Using more data points per PQ code may increase the quality of PQ
   * codebook but may also increase the build time. The parameter is applied to
   * both PQ codebook generation methods, i.e., PER_SUBSPACE and PER_CLUSTER. In
   * both cases, we will use `pq_book_size * max_train_points_per_pq_code`
...
struct cuvsIvfPqSearchParams {
  ...
  /**
   * The data type to use as the GEMM element type when searching the clusters
   * to probe.
   *
   ...
   *    (WARNING: int8_t variant degrades recall unless data is normalized and
   * low-dimensional)
   */
   ...
   * the main search kernel, but not to keep it too high to leave some memory to
   * be used as L1 cache. Note, this value is interpreted only as a hint.
   * Moreover, a GPU usually allows only a fixed set of cache configurations, so
   * the provided value is rounded up to the nearest configuration. Refer to the
 ...
 */
/**
 * @brief Struct to hold address of cuvs::neighbors::ivf_pq::index and its
...
/**
 * @brief Get the cluster centers corresponding to the lists in the original
 * space
 *
 * @param[in] index cuvsIvfPqIndex_t Built Ivf-Pq index
 * @param[out] centers Output tensor that will be populated with a non-owning
 * view of the data
 * @return cuvsError_t
 */
 ...
 *
 * @param[in] index cuvsIvfPqIndex_t Built Ivf-Pq index
 * @param[out] pq_centers Output tensor that will be populated with a non-owning
 * view of the data
 * @return cuvsError_t
 */
 ...
 * cuvsError_t build_status = cuvsIvfPqBuild(res, index_params, &dataset,
 * index);
 *
 * // de-allocate `index_params`, `index` and `res`
 * cuvsError_t params_destroy_status =
 * cuvsIvfPqIndexParamsDestroy(index_params); cuvsError_t index_destroy_status =
 * cuvsIvfPqIndexDestroy(index); cuvsError_t res_destroy_status =
 ...
 * @return cuvsError_t
 */
cuvsError_t cuvsIvfPqBuild(cuvsResources_t res, cuvsIvfPqIndexParams_t params,
                           DLManagedTensor *dataset, cuvsIvfPqIndex_t index);
/**
 * @}
 */
 ...
 *        `DLDeviceType` equal to `kDLCUDA`, `kDLCUDAHost`, `kDLCUDAManaged`.
 *        It is also important to note that the IVF-PQ Index must have been
 * built with the same type of `queries`, such that `index.dtype.code ==
 * queries.dl_tensor.dtype.code` Types for input are:
 *        1. `queries`: `kDLDataType.code == kDLFloat` and `kDLDataType.bits =
 * 32` or `kDLDataType.bits = 16`
 ...
 * &queries, &neighbors, &distances, filter);
 *
 * // de-allocate `search_params` and `res`
 * cuvsError_t params_destroy_status =
 * cuvsIvfPqSearchParamsDestroy(search_params); cuvsError_t res_destroy_status =
 * cuvsResourcesDestroy(res);
 * @endcode
 *
 * @param[in] res cuvsResources_t opaque C handle
...
cuvsError_t cuvsIvfPqSearch(cuvsResources_t res,
                            cuvsIvfPqSearchParams_t search_params,
                            cuvsIvfPqIndex_t index, DLManagedTensor *queries,
                            DLManagedTensor *neighbors,
                            DLManagedTensor *distances, cuvsFilter filter);
/**
 * @}
 */
 ...
 */
cuvsError_t cuvsIvfPqSerialize(cuvsResources_t res, const char *filename,
                               cuvsIvfPqIndex_t index);
 ...
 */
cuvsError_t cuvsIvfPqDeserialize(cuvsResources_t res, const char *filename,
                                 cuvsIvfPqIndex_t index);
/**
 * @}
 */
 ...
 * @return cuvsError_t
 */
cuvsError_t cuvsIvfPqExtend(cuvsResources_t res, DLManagedTensor *new_vectors,
