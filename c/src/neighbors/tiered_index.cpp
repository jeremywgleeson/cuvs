...
template <typename T>
void convert_c_index_params(cuvsTieredIndexParams params, int64_t n_rows,
                            int64_t dim, tiered_index::index_params<T> *out) {
...
template <typename T>
void *_build(cuvsResources_t res, cuvsTieredIndexParams params,
  ...
  RAFT_EXPECTS(dataset.shape != nullptr,
               "dataset should have an initialized shape");
  ...
  default:
    RAFT_FAIL("unsupported tiered index algorithm");
  }
}

template <typename UpstreamT>
void _search(cuvsResources_t res, void *params, cuvsTieredIndex index,
             DLManagedTensor *queries_tensor, DLManagedTensor *neighbors_tensor,
             DLManagedTensor *distances_tensor, cuvsFilter filter) {
  auto res_ptr = reinterpret_cast<raft::resources *>(res);
  auto index_ptr =
      reinterpret_cast<tiered_index::index<UpstreamT> *>(index.addr);

  auto search_params = typename UpstreamT::search_params_type();
  if (params != NULL) {
    if constexpr (std::is_same_v<typename UpstreamT::search_params_type,
                                 cagra::search_params>) {
      auto c_params = reinterpret_cast<cuvsCagraSearchParams *>(params);
      cagra::convert_c_search_params(*c_params, &search_params);
    }
    if constexpr (std::is_same_v<typename UpstreamT::search_params_type,
                                 ivf_flat::search_params>) {
  ...
  using distances_mdspan_type =
      raft::device_matrix_view<T, int64_t, raft::row_major>;
  auto queries_mds =
  ...
  if (filter.type == NO_FILTER) {
    tiered_index::search(*res_ptr, search_params, *index_ptr, queries_mds,
                         neighbors_mds, distances_mds);
    ...
    tiered_index::search(*res_ptr, search_params, *index_ptr, queries_mds,
                         neighbors_mds, distances_mds, bitmap_filter_obj);
  } else if (filter.type == BITSET) {
    using filter_mdspan_type =
        raft::device_vector_view<std::uint32_t, int64_t, raft::row_major>;
    auto removed_indices_tensor =
        reinterpret_cast<DLManagedTensor *>(filter.addr);
    auto removed_indices =
        cuvs::core::from_dlpack<filter_mdspan_type>(removed_indices_tensor);
    cuvs::core::bitset_view<std::uint32_t, int64_t> removed_indices_bitset(
        removed_indices, index_ptr->size());
    auto bitset_filter_obj =
        cuvs::neighbors::filtering::bitset_filter(removed_indices_bitset);

    tiered_index::search(*res_ptr, search_params, *index_ptr, queries_mds,
                         neighbors_mds, distances_mds, bitset_filter_obj);
  } else {
...
template <typename UpstreamT>
void _merge(cuvsResources_t res, cuvsTieredIndexParams params,
            cuvsTieredIndex_t *indices, size_t num_indices,
  ...
  for (size_t i = 0; i < num_indices; ++i) {
    ...
    auto idx_ptr =
        reinterpret_cast<cuvs::neighbors::tiered_index::index<UpstreamT> *>(
            indices[i]->addr);
    n_rows += idx_ptr->size();
    if (dim) {
      RAFT_EXPECTS(dim == idx_ptr->dim(),
                   "indices must all have the same dimensionality");
    } else {
      dim = idx_ptr->dim();
    }
  ...
  auto build_params =
      tiered_index::index_params<typename UpstreamT::index_params_type>();
  convert_c_index_params(params, n_rows, dim, &build_params);

  auto ptr = new cuvs::neighbors::tiered_index::index<UpstreamT>(
      cuvs::neighbors::tiered_index::merge(*res_ptr, build_params,
                                           cpp_indices));
    ...
    if (index.dtype.code == kDLFloat && index.dtype.bits == 32) {
      ...
      case CUVS_TIERED_INDEX_ALGO_CAGRA: {
        auto index_ptr = reinterpret_cast<
            tiered_index::index<cagra::index<float, uint32_t>> *>(index.addr);
        delete index_ptr;
        break;
      }
      case CUVS_TIERED_INDEX_ALGO_IVF_FLAT: {
        auto index_ptr = reinterpret_cast<
            tiered_index::index<ivf_flat::index<float, int64_t>> *>(index.addr);
        delete index_ptr;
        break;
      }
      case CUVS_TIERED_INDEX_ALGO_IVF_PQ: {
...
extern "C" cuvsError_t cuvsTieredIndexSearch(
    cuvsResources_t res, void *search_params, cuvsTieredIndex_t index_c_ptr,
    DLManagedTensor *queries_tensor, DLManagedTensor *neighbors_tensor,
    DLManagedTensor *distances_tensor, cuvsFilter filter)

{
  return cuvs::core::translate_exceptions([=] {
    ...
    switch (index.algo) {
    case CUVS_TIERED_INDEX_ALGO_CAGRA: {
      _search<cagra::index<float, uint32_t>>(res, search_params, index,
                                             queries_tensor, neighbors_tensor,
                                             distances_tensor, filter);
      break;
    }
    case CUVS_TIERED_INDEX_ALGO_IVF_FLAT: {
      _search<ivf_flat::index<float, int64_t>>(res, search_params, index,
                                               queries_tensor, neighbors_tensor,
                                               distances_tensor, filter);
      break;
    }
    case CUVS_TIERED_INDEX_ALGO_IVF_PQ: {
      _search<ivf_pq::typed_index<float, int64_t>>(
          res, search_params, index, queries_tensor, neighbors_tensor,
          distances_tensor, filter);
      break;
    }
    default:
...
cuvsTieredIndexParamsCreate(cuvsTieredIndexParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
...
extern "C" cuvsError_t
cuvsTieredIndexParamsDestroy(cuvsTieredIndexParams_t params) {
...
extern "C" cuvsError_t cuvsTieredIndexMerge(cuvsResources_t res,
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    case CUVS_TIERED_INDEX_ALGO_CAGRA: {
      _merge<cagra::index<float, uint32_t>>(res, *params, indices, num_indices,
                                            output_index);
      break;
    }
    case CUVS_TIERED_INDEX_ALGO_IVF_FLAT: {
      _merge<ivf_flat::index<float, int64_t>>(res, *params, indices,
                                              num_indices, output_index);
      break;
    }
    case CUVS_TIERED_INDEX_ALGO_IVF_PQ: {
      _merge<ivf_pq::typed_index<float, int64_t>>(res, *params, indices,
                                                  num_indices, output_index);
      break;
    }
    default:
