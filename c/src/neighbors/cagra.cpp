...
static void _set_graph_build_params(
    std::variant<
        ...
        &out_params,
    cuvsCagraIndexParams &params, cuvsCagraGraphBuildAlgo algo, int64_t n_rows,
    int64_t dim)

{
    ...
    if (params.graph_build_params) {
      ...
      if (ivf_params->ivf_pq_build_params) {
        ...
        pq_params.build_params.codebook_kind =
            static_cast<cuvs::neighbors::ivf_pq::codebook_gen>(
                bp->codebook_kind);
        pq_params.build_params.force_random_rotation =
            bp->force_random_rotation;
        ...
        pq_params.build_params.max_train_points_per_pq_code =
            bp->max_train_points_per_pq_code;
      }
      if (ivf_params->ivf_pq_search_params) {
        ...
        pq_params.search_params.internal_distance_dtype =
            sp->internal_distance_dtype;
        pq_params.search_params.preferred_shmem_carveout =
            sp->preferred_shmem_carveout;
      }
  ...
  case cuvsCagraGraphBuildAlgo::NN_DESCENT: {
    auto nn_params = cuvs::neighbors::nn_descent::index_params(
        params.intermediate_graph_degree, metric);
...
template <typename T>
void *_build(cuvsResources_t res, cuvsCagraIndexParams params,
  ...
  convert_c_index_params(params, dataset.shape[0], dataset.shape[1],
                         &index_params);

  if (cuvs::core::is_dlpack_device_compatible(dataset)) {
    using mdspan_type =
        raft::device_matrix_view<T const, int64_t, raft::row_major>;
...
template <typename T>
void *_from_args(cuvsResources_t res, cuvsDistanceType _metric,
  ...
  if (cuvs::core::is_dlpack_device_compatible(dataset)) {
    ...
    if (cuvs::core::is_dlpack_device_compatible(graph)) {
      using graph_mdspan_type =
          raft::device_matrix_view<uint32_t const, int64_t, raft::row_major>;
      auto graph_mds = cuvs::core::from_dlpack<graph_mdspan_type>(graph_tensor);
      index = new cuvs::neighbors::cagra::index<T, uint32_t>(*res_ptr, metric,
                                                             mds, graph_mds);
    } else {
      using graph_mdspan_type =
          raft::host_matrix_view<uint32_t const, int64_t, raft::row_major>;
      auto graph_mds = cuvs::core::from_dlpack<graph_mdspan_type>(graph_tensor);
      index = new cuvs::neighbors::cagra::index<T, uint32_t>(*res_ptr, metric,
                                                             mds, graph_mds);
    }
...
template <typename T>
void _extend(cuvsResources_t res, cuvsCagraExtendParams params,
             cuvsCagraIndex index, DLManagedTensor *additional_dataset_tensor) {
  ...
  if (cuvs::core::is_dlpack_device_compatible(dataset)) {
    ...
    RAFT_FAIL("Unsupported dataset DLtensor dtype: %d and bits: %d",
              dataset.dtype.code, dataset.dtype.bits);
  }
}

template <typename T, typename IdxT>
void _search(cuvsResources_t res, cuvsCagraSearchParams params,
             cuvsCagraIndex index, DLManagedTensor *queries_tensor,
             DLManagedTensor *neighbors_tensor,
             DLManagedTensor *distances_tensor, cuvsFilter filter) {
  auto res_ptr = reinterpret_cast<raft::resources *>(res);
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::cagra::index<T, uint32_t> *>(
          index.addr);
  ...
  if (filter.type == NO_FILTER) {
    cuvs::neighbors::cagra::search(*res_ptr, search_params, *index_ptr,
    ...
    const auto bitset_filter_obj = cuvs::neighbors::filtering::bitset_filter(
        filter_bst_type((std::uint32_t *)filter_mds.data_handle(),
...
void _search(cuvsResources_t res, cuvsCagraSearchParams params,
  ...
  if (neighbors_tensor->dl_tensor.dtype.code == kDLUInt &&
    ...
    _search<T, uint32_t>(res, params, index, queries_tensor, neighbors_tensor,
                         distances_tensor, filter);
  } else if (neighbors_tensor->dl_tensor.dtype.code == kDLInt &&
             neighbors_tensor->dl_tensor.dtype.bits == 64) {
    _search<T, int64_t>(res, params, index, queries_tensor, neighbors_tensor,
                        distances_tensor, filter);
  } else {
...
void _serialize(cuvsResources_t res, const char *filename,
  ...
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::cagra::index<T, uint32_t> *>(
          index->addr);
  cuvs::neighbors::cagra::serialize(*res_ptr, std::string(filename), *index_ptr,
                                    include_dataset);
}
...
void _serialize_to_hnswlib(cuvsResources_t res, const char *filename,
  ...
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::cagra::index<T, uint32_t> *>(
          index->addr);
  cuvs::neighbors::cagra::serialize_to_hnswlib(*res_ptr, std::string(filename),
                                               *index_ptr);
}
...
template <typename T>
void *_merge(cuvsResources_t res, cuvsCagraMergeParams params,
             cuvsCagraIndex_t *indices, size_t num_indices) {
  auto res_ptr = reinterpret_cast<raft::resources *>(res);
  ...
  merge_params_cpp.output_index_params.graph_degree =
      out_idx_params.graph_degree;
  ...
  _set_graph_build_params(
      merge_params_cpp.output_index_params.graph_build_params, out_idx_params,
      out_idx_params.build_algo, total_size, dim);
...
void get_dataset_view(cuvsCagraIndex_t index, DLManagedTensor *dataset) {
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::cagra::index<T, IdxT> *>(index->addr);
...
void convert_c_index_params(cuvsCagraIndexParams params, int64_t n_rows,
                            int64_t dim,
  ...
  _set_graph_build_params(out->graph_build_params, params, params.build_algo,
                          n_rows, dim);
  ...
  if (auto *cparams = params.compression; cparams != nullptr) {
    ...
    compression_params.vq_kmeans_trainset_fraction =
        cparams->vq_kmeans_trainset_fraction;
    compression_params.pq_kmeans_trainset_fraction =
        cparams->pq_kmeans_trainset_fraction;
...
void convert_c_search_params(cuvsCagraSearchParams params,
  ...
  out->hashmap_mode =
      static_cast<cuvs::neighbors::cagra::hash_mode>(params.hashmap_mode);
...
extern "C" cuvsError_t cuvsCagraIndexDestroy(cuvsCagraIndex_t index_c_ptr) {
  return cuvs::core::translate_exceptions([=] {
    ...
    if (index.dtype.code == kDLFloat && index.dtype.bits == 32) {
      auto index_ptr =
          reinterpret_cast<cuvs::neighbors::cagra::index<float, uint32_t> *>(
              index.addr);
      delete index_ptr;
    } else if (index.dtype.code == kDLFloat && index.dtype.bits == 16) {
      auto index_ptr =
          reinterpret_cast<cuvs::neighbors::cagra::index<half, uint32_t> *>(
              index.addr);
      delete index_ptr;
    } else if (index.dtype.code == kDLInt && index.dtype.bits == 8) {
      auto index_ptr =
          reinterpret_cast<cuvs::neighbors::cagra::index<int8_t, uint32_t> *>(
              index.addr);
      delete index_ptr;
    } else if (index.dtype.code == kDLUInt && index.dtype.bits == 8) {
      auto index_ptr =
          reinterpret_cast<cuvs::neighbors::cagra::index<uint8_t, uint32_t> *>(
              index.addr);
      delete index_ptr;
    }
    delete index_c_ptr;
...
extern "C" cuvsError_t cuvsCagraIndexGetDims(cuvsCagraIndex_t index,
                                             int64_t *dim) {
  return cuvs::core::translate_exceptions([=] {
    auto index_ptr =
        reinterpret_cast<cuvs::neighbors::cagra::index<float, uint32_t> *>(
...
extern "C" cuvsError_t cuvsCagraIndexGetSize(cuvsCagraIndex_t index,
                                             int64_t *size) {
  return cuvs::core::translate_exceptions([=] {
    auto index_ptr =
        reinterpret_cast<cuvs::neighbors::cagra::index<float, uint32_t> *>(
...
extern "C" cuvsError_t cuvsCagraIndexGetGraphDegree(cuvsCagraIndex_t index,
...
extern "C" cuvsError_t cuvsCagraIndexGetDataset(cuvsCagraIndex_t index,
...
extern "C" cuvsError_t cuvsCagraIndexGetGraph(cuvsCagraIndex_t index,
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    if (dataset.dtype.code == kDLFloat && dataset.dtype.bits == 32) {
      index->addr = reinterpret_cast<uintptr_t>(
          _build<float>(res, *params, dataset_tensor));
    } else if (dataset.dtype.code == kDLFloat && dataset.dtype.bits == 16) {
      index->addr = reinterpret_cast<uintptr_t>(
          _build<half>(res, *params, dataset_tensor));
...
extern "C" cuvsError_t cuvsCagraIndexFromArgs(cuvsResources_t res,
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    if (dataset.dtype.code == kDLFloat && dataset.dtype.bits == 32) {
      index->addr = reinterpret_cast<uintptr_t>(
          _from_args<float>(res, metric, graph_tensor, dataset_tensor));
    } else if (dataset.dtype.code == kDLFloat && dataset.dtype.bits == 16) {
      index->addr = reinterpret_cast<uintptr_t>(
          _from_args<half>(res, metric, graph_tensor, dataset_tensor));
    } else if (dataset.dtype.code == kDLInt && dataset.dtype.bits == 8) {
      index->addr = reinterpret_cast<uintptr_t>(
          _from_args<int8_t>(res, metric, graph_tensor, dataset_tensor));
    } else if (dataset.dtype.code == kDLUInt && dataset.dtype.bits == 8) {
      index->addr = reinterpret_cast<uintptr_t>(
          _from_args<uint8_t>(res, metric, graph_tensor, dataset_tensor));
    } else {
      RAFT_FAIL("Unsupported dataset DLtensor dtype: %d and bits: %d",
                dataset.dtype.code, dataset.dtype.bits);
    }
...
cuvsCagraExtend(cuvsResources_t res, cuvsCagraExtendParams_t params,
...
extern "C" cuvsError_t
cuvsCagraSearch(cuvsResources_t res, cuvsCagraSearchParams_t params,
                cuvsCagraIndex_t index_c_ptr, DLManagedTensor *queries_tensor,
                DLManagedTensor *neighbors_tensor,
                DLManagedTensor *distances_tensor, cuvsFilter filter) {
  return cuvs::core::translate_exceptions([=] {
    ...
    if (queries.dtype.code == kDLFloat && queries.dtype.bits == 32) {
      ...
      RAFT_FAIL("Unsupported queries DLtensor dtype: %d and bits: %d",
                queries.dtype.code, queries.dtype.bits);
    }
  });
}
...
extern "C" cuvsError_t cuvsCagraMerge(cuvsResources_t res,
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    RAFT_EXPECTS(indices != nullptr && num_indices > 0,
                 "indices array cannot be null or empty");
    // Use first index dtype as reference
    auto dtype = (*indices[0]).dtype;
    for (size_t i = 1; i < num_indices; ++i) {
      RAFT_EXPECTS((*indices[i]).dtype.code == dtype.code &&
                       (*indices[i]).dtype.bits == dtype.bits,
                   "All input indices must have the same data type");
      RAFT_EXPECTS((*indices[i]).addr != 0,
                   "All input indices must be built (non-empty)");
    }
    RAFT_EXPECTS(output_index != nullptr,
                 "Output index pointer must not be null");
    output_index->dtype = dtype; // output index type matches inputs
    // Dispatch based on data type
    if (dtype.code == kDLFloat && dtype.bits == 32) {
      output_index->addr = reinterpret_cast<uintptr_t>(
          _merge<float>(res, *params, indices, num_indices));
    } else if (dtype.code == kDLFloat && dtype.bits == 16) {
      output_index->addr = reinterpret_cast<uintptr_t>(
          _merge<half>(res, *params, indices, num_indices));
    } else if (dtype.code == kDLInt && dtype.bits == 8) {
      output_index->addr = reinterpret_cast<uintptr_t>(
          _merge<int8_t>(res, *params, indices, num_indices));
    } else if (dtype.code == kDLUInt && dtype.bits == 8) {
      output_index->addr = reinterpret_cast<uintptr_t>(
          _merge<uint8_t>(res, *params, indices, num_indices));
    } else {
      RAFT_FAIL("Unsupported index data type: code=%d, bits=%d", dtype.code,
                dtype.bits);
    }
  });
}
...
extern "C" cuvsError_t
cuvsCagraIndexParamsCreate(cuvsCagraIndexParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
...
extern "C" cuvsError_t
cuvsCagraIndexParamsDestroy(cuvsCagraIndexParams_t params) {
  return cuvs::core::translate_exceptions([=] {
...
extern "C" cuvsError_t
cuvsCagraCompressionParamsCreate(cuvsCagraCompressionParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
    auto ps = cuvs::neighbors::vpq_params();
    *params = new cuvsCagraCompressionParams{
        .pq_bits = ps.pq_bits,
        .pq_dim = ps.pq_dim,
...
extern "C" cuvsError_t
cuvsCagraCompressionParamsDestroy(cuvsCagraCompressionParams_t params) {
  return cuvs::core::translate_exceptions([=] { delete params; });
}

extern "C" cuvsError_t
cuvsCagraExtendParamsCreate(cuvsCagraExtendParams_t *params) {
  return cuvs::core::translate_exceptions(
      [=] { *params = new cuvsCagraExtendParams{.max_chunk_size = 0}; });
}

extern "C" cuvsError_t
cuvsCagraExtendParamsDestroy(cuvsCagraExtendParams_t params) {
  return cuvs::core::translate_exceptions([=] { delete params; });
}

extern "C" cuvsError_t
cuvsCagraSearchParamsCreate(cuvsCagraSearchParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
...
extern "C" cuvsError_t
cuvsCagraSearchParamsDestroy(cuvsCagraSearchParams_t params) {
  return cuvs::core::translate_exceptions([=] { delete params; });
}

extern "C" cuvsError_t
cuvsCagraMergeParamsCreate(cuvsCagraMergeParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
...
extern "C" cuvsError_t
cuvsCagraMergeParamsDestroy(cuvsCagraMergeParams_t params) {
  return cuvs::core::translate_exceptions([=] {
...
extern "C" cuvsError_t cuvsCagraDeserialize(cuvsResources_t res,
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    auto dtype = raft::detail::numpy_serializer::parse_descr(
        std::string(dtype_string, 4));
    ...
    if (dtype.kind == 'f' && dtype.itemsize == 4) {
      ...
      index->addr =
          reinterpret_cast<uintptr_t>(_deserialize<int8_t>(res, filename));
      index->dtype.code = kDLInt;
    } else if (dtype.kind == 'u' && dtype.itemsize == 1) {
      index->addr =
          reinterpret_cast<uintptr_t>(_deserialize<uint8_t>(res, filename));
