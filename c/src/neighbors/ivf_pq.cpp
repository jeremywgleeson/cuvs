...
void convert_c_index_params(cuvsIvfPqIndexParams params,
  ...
  out->codebook_kind = static_cast<cuvs::neighbors::ivf_pq::codebook_gen>(
...
void *_build(cuvsResources_t res, cuvsIvfPqIndexParams params,
  ...
  auto index =
      new cuvs::neighbors::ivf_pq::index<IdxT>(*res_ptr, build_params, dim);

  if (cuvs::core::is_dlpack_device_compatible(dataset)) {
    using mdspan_type =
        raft::device_matrix_view<const T, IdxT, raft::row_major>;
...
template <typename T, typename IdxT>
void _search(cuvsResources_t res, cuvsIvfPqSearchParams params,
             cuvsIvfPqIndex index, DLManagedTensor *queries_tensor,
             DLManagedTensor *neighbors_tensor,
             DLManagedTensor *distances_tensor, cuvsFilter filter) {
  auto res_ptr = reinterpret_cast<raft::resources *>(res);
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::ivf_pq::index<IdxT> *>(index.addr);
  ...
  using neighbors_mdspan_type =
      raft::device_matrix_view<IdxT, IdxT, raft::row_major>;
  using distances_mdspan_type =
      raft::device_matrix_view<float, IdxT, raft::row_major>;
  auto queries_mds =
  ...
  if (filter.type == NO_FILTER) {
    cuvs::neighbors::ivf_pq::search(*res_ptr, search_params, *index_ptr,
    ...
    const auto bitmap_filter_obj = cuvs::neighbors::filtering::bitmap_filter(
        filter_bmp_type((std::uint32_t *)filter_mds.data_handle(),
                        queries_mds.extent(0), index_ptr->size()));
    cuvs::neighbors::ivf_pq::search(*res_ptr, search_params, *index_ptr,
                                    queries_mds, neighbors_mds, distances_mds,
                                    bitmap_filter_obj);
    ...
    auto filter_mds =
        cuvs::core::from_dlpack<filter_mdspan_type>(filter_tensor);
    const auto bitset_filter_obj =
        cuvs::neighbors::filtering::bitset_filter(filter_bst_type(
            (std::uint32_t *)filter_mds.data_handle(), index_ptr->size()));
    cuvs::neighbors::ivf_pq::search(*res_ptr, search_params, *index_ptr,
                                    queries_mds, neighbors_mds, distances_mds,
                                    bitset_filter_obj);
  } else {
...
void _serialize(cuvsResources_t res, const char *filename,
  ...
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::ivf_pq::index<IdxT> *>(index.addr);
  cuvs::neighbors::ivf_pq::serialize(*res_ptr, std::string(filename),
                                     *index_ptr);
}
...
template <typename T, typename IdxT>
void _extend(cuvsResources_t res, DLManagedTensor *new_vectors,
             DLManagedTensor *new_indices, cuvsIvfPqIndex index) {
  auto res_ptr = reinterpret_cast<raft::resources *>(res);
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::ivf_pq::index<IdxT> *>(index.addr);

  bool on_device =
      cuvs::core::is_dlpack_device_compatible(new_vectors->dl_tensor);
  if (on_device !=
      cuvs::core::is_dlpack_device_compatible(new_indices->dl_tensor)) {
    RAFT_FAIL(
        "extend inputs must both either be on device memory or host memory");
  }

  if (on_device) {
    using vectors_mdspan_type =
        raft::device_matrix_view<const T, IdxT, raft::row_major>;
    using indices_mdspan_type = raft::device_vector_view<IdxT, IdxT>;
    auto vectors_mds =
        cuvs::core::from_dlpack<vectors_mdspan_type>(new_vectors);
    auto indices_mds =
        cuvs::core::from_dlpack<indices_mdspan_type>(new_indices);
    cuvs::neighbors::ivf_pq::extend(*res_ptr, vectors_mds, indices_mds,
                                    index_ptr);
  } else {
    using vectors_mdspan_type =
        raft::host_matrix_view<const T, IdxT, raft::row_major>;
...
void _get_centers(cuvsIvfPqIndex index, DLManagedTensor *output) {
  ...
  // mimic the behaviour of `extract_centers` without performing a copy of the
  // data
  auto strided_centers = raft::make_device_strided_matrix_view(
      centers.data_handle(), centers.extent(0), index_ptr->dim(),
      index_ptr->dim_ext());
...
extern "C" cuvsError_t cuvsIvfPqIndexDestroy(cuvsIvfPqIndex_t index_c_ptr) {
  return cuvs::core::translate_exceptions([=] {
    ...
    auto index_ptr =
        reinterpret_cast<cuvs::neighbors::ivf_pq::index<int64_t> *>(index.addr);
    delete index_ptr;
    delete index_c_ptr;
  });
}
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    if (dataset.dtype.code == kDLFloat && dataset.dtype.bits == 32) {
      index->addr = reinterpret_cast<uintptr_t>(
          _build<float, int64_t>(res, *params, dataset_tensor));
    } else if (dataset.dtype.code == kDLFloat && dataset.dtype.bits == 16) {
      index->addr = reinterpret_cast<uintptr_t>(
          _build<half, int64_t>(res, *params, dataset_tensor));
    } else if (dataset.dtype.code == kDLInt && dataset.dtype.bits == 8) {
      index->addr = reinterpret_cast<uintptr_t>(
          _build<int8_t, int64_t>(res, *params, dataset_tensor));
    } else if (dataset.dtype.code == kDLUInt && dataset.dtype.bits == 8) {
      index->addr = reinterpret_cast<uintptr_t>(
          _build<uint8_t, int64_t>(res, *params, dataset_tensor));
    } else {
...
cuvsIvfPqSearch(cuvsResources_t res, cuvsIvfPqSearchParams_t params,
                cuvsIvfPqIndex_t index_c_ptr, DLManagedTensor *queries_tensor,
                DLManagedTensor *neighbors_tensor,
                DLManagedTensor *distances_tensor, cuvsFilter filter) {
  return cuvs::core::translate_exceptions([=] {
    ...
    if (queries.dtype.code == kDLFloat && queries.dtype.bits == 32) {
      _search<float, int64_t>(res, *params, index, queries_tensor,
                              neighbors_tensor, distances_tensor, filter);
    } else if (queries.dtype.code == kDLFloat && queries.dtype.bits == 16) {
      _search<half, int64_t>(res, *params, index, queries_tensor,
                             neighbors_tensor, distances_tensor, filter);
    } else if (queries.dtype.code == kDLInt && queries.dtype.bits == 8) {
      _search<int8_t, int64_t>(res, *params, index, queries_tensor,
                               neighbors_tensor, distances_tensor, filter);
    } else if (queries.dtype.code == kDLUInt && queries.dtype.bits == 8) {
      _search<uint8_t, int64_t>(res, *params, index, queries_tensor,
                                neighbors_tensor, distances_tensor, filter);
    } else {
...
extern "C" cuvsError_t
cuvsIvfPqIndexParamsCreate(cuvsIvfPqIndexParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
    *params =
        new cuvsIvfPqIndexParams{.metric = L2Expanded,
                                 .metric_arg = 2.0f,
...
extern "C" cuvsError_t
cuvsIvfPqIndexParamsDestroy(cuvsIvfPqIndexParams_t params) {
  return cuvs::core::translate_exceptions([=] { delete params; });
}

extern "C" cuvsError_t
cuvsIvfPqSearchParamsCreate(cuvsIvfPqSearchParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
...
extern "C" cuvsError_t
cuvsIvfPqSearchParamsDestroy(cuvsIvfPqSearchParams_t params) {
  return cuvs::core::translate_exceptions([=] { delete params; });
}
...
extern "C" cuvsError_t cuvsIvfPqDeserialize(cuvsResources_t res,
  ...
  return cuvs::core::translate_exceptions([=] {
    index->addr =
        reinterpret_cast<uintptr_t>(_deserialize<int64_t>(res, filename));
  });
}
...
extern "C" cuvsError_t cuvsIvfPqSerialize(cuvsResources_t res,
                                          ...
                                          cuvsIvfPqIndex_t index) {
  return cuvs::core::translate_exceptions(
      [=] { _serialize<int64_t>(res, filename, *index); });
}
...
extern "C" cuvsError_t cuvsIvfPqExtend(cuvsResources_t res,
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    if (vectors.dtype.code == kDLFloat && vectors.dtype.bits == 32) {
      ...
      RAFT_FAIL("Unsupported index dtype: %d and bits: %d", vectors.dtype.code,
                vectors.dtype.bits);
    }
  });
}

extern "C" cuvsError_t cuvsIvfPqIndexGetNLists(cuvsIvfPqIndex_t index,
                                               int64_t *n_lists) {
  return cuvs::core::translate_exceptions([=] {
    auto index_ptr =
        reinterpret_cast<cuvs::neighbors::ivf_pq::index<int64_t> *>(
            index->addr);
    *n_lists = index_ptr->n_lists();
  });
}

extern "C" cuvsError_t cuvsIvfPqIndexGetDim(cuvsIvfPqIndex_t index,
                                            int64_t *dim) {
  return cuvs::core::translate_exceptions([=] {
    auto index_ptr =
        reinterpret_cast<cuvs::neighbors::ivf_pq::index<int64_t> *>(
...
extern "C" cuvsError_t cuvsIvfPqIndexGetSize(cuvsIvfPqIndex_t index,
                                             int64_t *size) {
  return cuvs::core::translate_exceptions([=] {
    auto index_ptr =
        reinterpret_cast<cuvs::neighbors::ivf_pq::index<int64_t> *>(
...
extern "C" cuvsError_t cuvsIvfPqIndexGetCenters(cuvsIvfPqIndex_t index,
                                                DLManagedTensor *centers) {
  return cuvs::core::translate_exceptions(
      [=] { _get_centers<int64_t>(*index, centers); });
}

extern "C" cuvsError_t cuvsIvfPqIndexGetPqCenters(cuvsIvfPqIndex_t index,
                                                  DLManagedTensor *pq_centers) {
  return cuvs::core::translate_exceptions(
      [=] { _get_pq_centers<int64_t>(*index, pq_centers); });
}
