...
void *_build(cuvsResources_t res, cuvsIvfFlatIndexParams params,
...
template <typename T, typename IdxT>
void _search(cuvsResources_t res, cuvsIvfFlatSearchParams params,
             cuvsIvfFlatIndex index, DLManagedTensor *queries_tensor,
             DLManagedTensor *neighbors_tensor,
             DLManagedTensor *distances_tensor, cuvsFilter filter) {
  auto res_ptr = reinterpret_cast<raft::resources *>(res);
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::ivf_flat::index<T, IdxT> *>(index.addr);
  ...
  using queries_mdspan_type =
      raft::device_matrix_view<T const, IdxT, raft::row_major>;
  ...
  if (filter.type == NO_FILTER) {
    cuvs::neighbors::ivf_flat::search(*res_ptr, search_params, *index_ptr,
                                      queries_mds, neighbors_mds,
                                      distances_mds);
...
void _serialize(cuvsResources_t res, const char *filename,
  ...
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::ivf_flat::index<T, IdxT> *>(index.addr);
  cuvs::neighbors::ivf_flat::serialize(*res_ptr, std::string(filename),
                                       *index_ptr);
}
...
void _extend(cuvsResources_t res, DLManagedTensor *new_vectors,
             DLManagedTensor *new_indices, cuvsIvfFlatIndex index) {
  auto res_ptr = reinterpret_cast<raft::resources *>(res);
  auto index_ptr =
      reinterpret_cast<cuvs::neighbors::ivf_flat::index<T, IdxT> *>(index.addr);
  ...
  cuvs::neighbors::ivf_flat::extend(*res_ptr, vectors_mds, indices_mds,
                                    index_ptr);
}
...
extern "C" cuvsError_t cuvsIvfFlatIndexDestroy(cuvsIvfFlatIndex_t index_c_ptr) {
  return cuvs::core::translate_exceptions([=] {
    ...
    if (index.dtype.code == kDLFloat && index.dtype.bits == 32) {
      auto index_ptr =
          reinterpret_cast<cuvs::neighbors::ivf_flat::index<float, int64_t> *>(
              index.addr);
      delete index_ptr;
    } else if (index.dtype.code == kDLFloat && index.dtype.bits == 16) {
      auto index_ptr =
          reinterpret_cast<cuvs::neighbors::ivf_flat::index<half, int64_t> *>(
              index.addr);
      delete index_ptr;
    } else if (index.dtype.code == kDLInt) {
      auto index_ptr =
          reinterpret_cast<cuvs::neighbors::ivf_flat::index<int8_t, int64_t> *>(
              index.addr);
      delete index_ptr;
    } else if (index.dtype.code == kDLUInt) {
      auto index_ptr = reinterpret_cast<
          cuvs::neighbors::ivf_flat::index<uint8_t, int64_t> *>(index.addr);
      delete index_ptr;
    }
    delete index_c_ptr;
  });
...
cuvsIvfFlatSearch(cuvsResources_t res, cuvsIvfFlatSearchParams_t params,
...
extern "C" cuvsError_t
cuvsIvfFlatIndexParamsCreate(cuvsIvfFlatIndexParams_t *params) {
  return cuvs::core::translate_exceptions([=] {
    *params =
        new cuvsIvfFlatIndexParams{.metric = L2Expanded,
                                   .metric_arg = 2.0f,
...
extern "C" cuvsError_t
cuvsIvfFlatIndexParamsDestroy(cuvsIvfFlatIndexParams_t params) {
...
extern "C" cuvsError_t cuvsIvfFlatDeserialize(cuvsResources_t res,
  ...
  return cuvs::core::translate_exceptions([=] {
    ...
    if (dtype.kind == 'f' && dtype.itemsize == 4) {
      ...
      index->addr = reinterpret_cast<uintptr_t>(
          _deserialize<int8_t, int64_t>(res, filename));
      index->dtype.code = kDLInt;
    } else if (dtype.kind == 'u' && dtype.itemsize == 1) {
      index->addr = reinterpret_cast<uintptr_t>(
          _deserialize<uint8_t, int64_t>(res, filename));
      index->dtype.code = kDLUInt;
    } else {
      RAFT_FAIL("Unsupported dtype in file %s itemsize %i kind %i", filename,
                dtype.itemsize, dtype.kind);
    }
  });
}
...
extern "C" cuvsError_t cuvsIvfFlatIndexGetNLists(cuvsIvfFlatIndex_t index,
                                                 int64_t *n_lists) {
  return cuvs::core::translate_exceptions([=] {
    if (index->dtype.code == kDLFloat && index->dtype.bits == 32) {
      auto index_ptr =
...
extern "C" cuvsError_t cuvsIvfFlatIndexGetDim(cuvsIvfFlatIndex_t index,
                                              int64_t *dim) {
  return cuvs::core::translate_exceptions([=] {
    if (index->dtype.code == kDLFloat && index->dtype.bits == 32) {
      auto index_ptr =
